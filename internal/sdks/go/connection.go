...
func Connect(conf *ConfigOption) (*Connection, error) {
	...
	conn.SetDeadline(time.Now().Add(time.Second * message.MAX_IDLE_TIME))
	...
	for {
		...
		case mqtt.TypeOfPublish:
			...
			if len(pl) > 0 {
				switch pl[0] {
				case message.COUNT:
					count := message.UnpackMsgCount(pl[1:])
					c.unreadh(m.Topic, count)
				case message.PUB_BATCH:
					msgs, _ := message.UnpackPubBatch(pl[1:])
					for _, m := range msgs {
						c.pubmsgh(m)
					}
				case message.PUB_ONE:
					msg, _ := message.UnpackPub(pl[1:])
					c.pubmsgh(msg)
				}
			}
